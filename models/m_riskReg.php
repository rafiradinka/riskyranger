...
class RiskReg {
    ...
    public function edit($id_risk, $tujuan, $kode_risk, $jenis_risk, $bisnis_risk, $sumber_risk, $uraian_risk, $penyebab_risk, $kualitatif_risk, $kuantitatif_risk, $risk_owner, $unit_terkait){
        $db = $this->connection;
        $stmt = $db->prepare("UPDATE tb_risk SET tujuan = ?, kode_risk = ?, jenis_risk = ?, bisnis_risk = ?, sumber_risk = ?, uraian_risk = ?, penyebab_risk = ?, kualitatif_risk = ?, kuantitatif_risk = ?, risk_owner = ?, unit_terkait = ? WHERE id_risk = ?");
        $stmt->bind_param('sssssssssssi', $tujuan, $kode_risk, $jenis_risk, $bisnis_risk, $sumber_risk, $uraian_risk, $penyebab_risk, $kualitatif_risk, $kuantitatif_risk, $risk_owner, $unit_terkait,  $id_risk);
        $stmt->execute() or die($db->error);
        $stmt->close();
    }
